import type { ResponseAladinSearchBook } from './types';
import type { AxiosError } from 'axios';
import { HttpService } from '@nestjs/axios';
import { Injectable, Logger, NotFoundException } from '@nestjs/common';
import { ConfigService } from '@nestjs/config';
import { catchError, firstValueFrom } from 'rxjs';
import { AladinLookupResDto } from './dto/aladin';

@Injectable()
export class AladinSearchService {
  private readonly logger = new Logger(AladinSearchService.name);
  ...
  public async searchAladinBook(isbn: string): Promise<AladinLookupResDto | null> {
      ...
      this.httpService
        ...
        .pipe(
          catchError((error: AxiosError) => {
            this.logger.error(`알라딘 도서 검색 API 호출 오류 : ${error.message}, ${error.stack}`);
            throw error;
          }),
        ),
    );
